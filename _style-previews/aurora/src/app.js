...
function bindEvents() {
  ...
  refs.employeeDetail.addEventListener("submit", (event) => {
    ...
    if (event.target.id === "chatForm") {
      ...
      if (!message) return;
      const reply = createSimulatedReply(getSelectedEmployee(), message);
...
function renderAvatarVisual(employee) {
  const actualAvatarSrc = employee.avatar?.src;
  const proxyAvatarSrc = employee.avatar?.proxySrc;
  const avatarSrc = actualAvatarSrc || proxyAvatarSrc;
  const isProxyAvatar = !actualAvatarSrc && Boolean(proxyAvatarSrc);
  const proxyVariant = isProxyAvatar ? employee.avatar?.proxyVariant ?? "proxy-default" : "";
  const proxyBadge = isProxyAvatar ? employee.avatar?.proxyBadge ?? "" : "";
  const fallback = `<span class="agent-avatar-fallback">${renderEmployeeAvatar(employee)}</span>`;
  const resolvedAvatarSrc = avatarSrc ? new URL(avatarSrc, window.location.href).href : "";
  if (!avatarSrc || failedAvatarSrcs.has(avatarSrc) || failedAvatarSrcs.has(resolvedAvatarSrc)) return fallback;

  return `
    <span class="avatar-image-wrap ${isProxyAvatar ? `is-proxy-avatar ${escapeHtml(proxyVariant)}` : "is-actual-avatar"}" ${
      proxyBadge ? `data-role-badge="${escapeHtml(proxyBadge)}"` : ""
    }>
      <span class="avatar-image-fallback">${fallback}</span>
      <img
  ...
  refs.employeeDetail.innerHTML = `
    ...
    <div class="detail-body">
      <div class="detail-avatar-wrap">
        ${renderEmployeeToken(employee, 0, { isPreview: true, hideBubble: true })}
      </div>
